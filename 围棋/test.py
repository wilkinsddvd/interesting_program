...
window = tk.Tk()  # 建立一个窗口
window.title('围棋游戏登录')
window.geometry('1000x1920')  # 窗口大小

# 画布
canvas = tk.Canvas(window, height=1087, width=1078)
# 加载图片
im = Image.open("Pictures/01.png")
image_file = ImageTk.PhotoImage(im)
# image_file = tk.PhotoImage(file='images/01.gif')
image = canvas.create_image(400, 440, anchor='center', image=image_file)
...
def usr_login():
    ...
    if usr_name in usrs_info:
        if usr_pwd == usrs_info[usr_name]:
            tk.messagebox.showinfo('登陆成功', '恭喜您的密码正确！请手动关闭当前页面，开始游戏吧！')

        else:
            tk.messagebox.showinfo('登陆失败', '友情提示：您的用户名或密码存在错误，请重试！')
    else:
        is_sign_up = tk.messagebox.askyesno('友情提示', '您的用户名未注册，请先进行注册！')
        if is_sign_up:
            usr_sign_up()
...
def usr_sign_up():
    def sign_to_Mofan_Python():
        ...
        with open('usrs_info.pickle', 'rb') as usr_file:
            ...
            if np != npf:
                tk.messagebox.showerror('友情提示', '两次密码不一致！')
            elif nn in exist_usr_info:
                tk.messagebox.showerror('友情提示', '该用户名已注册！')
            else:
                exist_usr_info[nn] = np
                with open('usrs_info.pickle', 'wb') as usr_file:
                    pickle.dump(exist_usr_info, usr_file)
                tk.messagebox.showinfo('欢迎', '您已成功注册！')
                window_sign_up.destroy()

    # 点击注册之后，会弹出这个窗口界面。
    window_sign_up = tk.Toplevel(window)
    window_sign_up.title('围棋游戏注册')
    window_sign_up.geometry('400x500')  # 注册页面大小

    # 用户名框--这里输入用户名框。
    new_name = tk.StringVar()
    new_name.set('用户名')  # 设置的是默认值
...
class Program(Tk):
   ...
   def __init__(self,my_mode_num=19):
      ...
      self.last_3_positions=copy.deepcopy(self.positions) # 快照1
      self.last_2_positions=copy.deepcopy(self.positions) # 快照2
      self.last_1_positions=copy.deepcopy(self.positions) # 快照3
            ...
            if self.positions[y][x]==0:
               ...
               if not self.last_2_positions==self.positions:
                  ...
                  if len(deadlist)>0 or self.if_dead([[x,y]],self.present+1,[x,y])==False:
                     ...
                     self.bell()
                     self.showwarningbox('友情提醒',"因为无气，不能在此处落子！")
               else:
                  # 重复棋局，警告打劫
                  self.positions[y][x]=0
                  self.canvas_bottom.delete('position'+str(x)+str(y))
                  ...
                  self.bell()
                  self.showwarningbox("友情提醒","因为打劫，不能在此处落子！")
            else:
               # 覆盖，声音警告
               self.bell()
         else:
